...
fun registerAllTools(handler: AIToolHandler, context: Context) {
        ...
        executor = { tool ->
            ...
            try {
                ...
                ToolResult(
                    toolName = tool.name,
                    success = true,
                    result = StringResultData("已播报: $text")
                )
            } catch (e: Exception) {
                ToolResult(
                    toolName = tool.name,
    ...
    // 检测权限通道 - 返回无障碍/Shizuku/Root 状态
    handler.registerTool(
        name = "get_permission_status",
        descriptionGenerator = { _ -> "检测当前设备可用的控制权限（无障碍、Shizuku、Root）" },
        executor = { tool ->
            val mgr = com.ai.assistance.operit.data.preferences.ElderlyModeManager.getInstance(context)
            val info = mgr.detectPermissionChannels()
            val result = buildString {
                appendLine("权限通道检测结果:")
                appendLine("- 无障碍服务: ${if (info.accessibilityEnabled) "✅ 已开启" else "❌ 未开启"}")
                appendLine("- Shizuku: ${if (info.shizukuAvailable) "✅ 可用" else "❌ 不可用"}")
                appendLine("- Root: ${if (info.rootAvailable) "✅ 可用" else "❌ 不可用"}")
                appendLine("- 推荐模式: ${when(info.recommendedMode) {
                    "accessibility" -> "无障碍服务"
                    "shizuku" -> "Shizuku"
                    "root" -> "Root"
                    else -> "无可用通道，请先开启无障碍"
    ...
    handler.registerTool(
        ...
        descriptionGenerator = { _ -> "打开系统无障碍设置页面，引导用户授权" },
        executor = { tool ->
            try {
                val mgr = com.ai.assistance.operit.data.preferences.ElderlyModeManager.getInstance(context)
                mgr.openAccessibilitySettings()
                ToolResult(
                    toolName = tool.name,
                    success = true,
                    result = StringResultData("已打开无障碍设置页面，请找到 Operit 并开启服务")
                )
            } catch (e: Exception) {
                ToolResult(
                    toolName = tool.name,
    ...
    // 查询流程库 - 按模式/应用/目的筛选
    handler.registerTool(
        name = "query_flow_library",
        descriptionGenerator = { tool ->
            val mode = tool.parameters.find { it.name == "mode" }?.value ?: ""
            val app = tool.parameters.find { it.name == "target_app" }?.value ?: ""
            val purpose = tool.parameters.find { it.name == "purpose" }?.value ?: ""
            "查询流程库: 模式=$mode 应用=$app 目的=$purpose"
        },
        executor = { tool ->
            val mode = tool.parameters.find { it.name == "mode" }?.value
            val app = tool.parameters.find { it.name == "target_app" }?.value
            ...
            val results = com.ai.assistance.operit.data.model.FlowLibraryCatalog.query(mode, app, purpose)

            val result = buildString {
                appendLine("找到 ${results.size} 个匹配流程:")
                results.forEachIndexed { i, flow ->
                    appendLine()
                    appendLine("[${i+1}] ${flow.name} (${flow.id})")
                    appendLine("    模式: ${flow.mode} | 应用: ${flow.targetApp} | 目的: ${flow.purpose}")
                    appendLine("    描述: ${flow.description}")
                    appendLine("    预计: ${flow.estimatedSeconds}秒 | 难度: ${flow.difficulty}")
                    appendLine("    步骤:")
                    flow.steps.forEachIndexed { j, step ->
                        appendLine("      ${j+1}. [${step.action}] ${step.description}${if (step.text.isNotEmpty()) " -> \"${step.text}\"" else ""}")
                    }
                }
            }
            ToolResult(
    ...
    // 获取流程分类列表
    handler.registerTool(
        name = "list_flow_categories",
        descriptionGenerator = { _ -> "列出流程库所有分类（模式、应用、目的）" },
        executor = { tool ->
            val cats = com.ai.assistance.operit.data.model.FlowLibraryCatalog.listCategories()
            val result = buildString {
                appendLine("流程库分类:")
                cats.forEach { (key, values) ->
