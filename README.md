...
<details>
<summary><b>🔄 多权限优先级与多层启动配置</b></summary>

- **多权限并行调度**：AI 可同时调用多个工具执行任务，按批次并行处理，提升复杂任务的执行效率
- **优先级排序机制**：每个权限配置可设置优先级，高优先级工具优先执行，确保关键路径工具先被调用
- **失败自动回退**：当某工具因权限不足或执行失败时，自动切换到下一个权限配置重试，保障任务连续性
- **可视化配置界面**：新增「多权限配置」页面，支持添加、删除、排序和编辑多个权限方案，实时预览执行顺序

</details>

<details>
<summary><b>🧠 思考过程实时显示</b></summary>

- **流式思考展示**：在对话界面中实时展开 AI 的思考过程（thinking blocks），透明展示模型的推理步骤
- **智能内容提取**：ThinkingContentExtractor 自动从流式输出中识别并分离思考内容与正式回复
- **可折叠面板**：思考过程以可折叠卡片形式展示，支持展开查看完整推理、折叠以保持界面简洁
- **打字机动画**：思考内容逐字流式渲染，配合缩放动画指示器，提升实时感和沉浸体验

</details>

<details>
<summary><b>🪟 悬浮窗流式信息面板</b></summary>

- **AI 状态实时面板**：悬浮窗新增当前对话 AI 信息展示，包括模型名称、处理状态（连接中/接收中/空闲）
- **流式消息预览**：在悬浮窗中即可看到 AI 回复的流式输出，无需打开主界面
- **最新消息高亮**：自动定位到最新消息，配合打字机指示器，直观感知 AI 正在回复
- **多消息滚动列表**：悬浮窗内支持查看多条对话消息，按时间倒序排列，随时回顾上下文
...
- **v1.13.0 · 2026-09**：新增多权限优先级并行调度与失败回退、思考过程流式显示、悬浮窗 AI 信息与流式消息面板
